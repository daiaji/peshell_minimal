...
function M.initialize()
    print("PE: Initializing user environment folders...")

    local user_profile = os.getenv("USERPROFILE")
    if not user_profile then
        print("Error: USERPROFILE environment variable is not set. Cannot initialize folders.")
        return
    end
    ...
    for _, dir in ipairs(directories) do
        ...
        if not success then
            ...
            if not (err and err:find("exists")) then
                print("Warning: Could not create directory '" .. full_path .. "': " .. tostring(err))
            end
        end
    end
    ...
    print("PE: User environment folders initialized.")
end
