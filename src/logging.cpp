...
namespace
{
    ...
    void apply_log_settings(const std::filesystem::path& config_path)
    {
        ...
        if (format_str == "json") {
            spdlog::default_logger()->set_formatter(std::make_unique<spdlog::pattern_formatter>(
                JSON_LOG_PATTERN, spdlog::pattern_time_type::local));
        } else {
            spdlog::default_logger()->set_formatter(std::make_unique<spdlog::pattern_formatter>(
                PLAIN_LOG_PATTERN, spdlog::pattern_time_type::local));
        }
    ...
    {
        // ==================== [核心修正 2/3] 将句柄存入全局变量 ====================
        g_hConfigDirHandle = CreateFileW(
            g_config_dir_wstr.c_str(),
            FILE_LIST_DIRECTORY,
        ...
        if (g_hConfigDirHandle == INVALID_HANDLE_VALUE) {
            spdlog::error("Failed to get handle to config directory for monitoring. Dynamic reload disabled. Error: {}", GetLastError());
            return;
        }
        ...
        while (!g_shutdown_flag)
        {
            if (ReadDirectoryChangesW(
                g_hConfigDirHandle, buffer, sizeof(buffer), FALSE,
            ...
            ))
            {
                ...
                spdlog::warn("ReadDirectoryChangesW failed. Dynamic reload may be interrupted. Error: {}", GetLastError());
                Sleep(5000);
            }
        }
