...
namespace
{
    ...
    // ==================== [核心修正 1/3] 新增全局句柄用于中断IO ====================
    HANDLE g_hConfigDirHandle = INVALID_HANDLE_VALUE;

    // 定义两种日志格式模板
    const char* PLAIN_LOG_PATTERN = "[%Y-%m-%d %H:%M:%S.%f] [pid:%P] [thread:%t] [%^%l%$] %v";
    const char* JSON_LOG_PATTERN =
        R"({"timestamp":"%Y-%m-%d %H:%M:%S.%f","level":"%l","thread":%t,"pid":%P,"message":"%v"})";

    // Helper: 字符串级别转换为spdlog枚举
    ...
    {
        std::string level_str  = "info";  // 默认级别
        std::string format_str = "plain"; // 默认格式
        ...
        if (format_str == "json")
        {
            spdlog::default_logger()->set_formatter(
                std::make_unique<spdlog::pattern_formatter>(JSON_LOG_PATTERN, spdlog::pattern_time_type::local));
        }
        else
        {
            spdlog::default_logger()->set_formatter(
                std::make_unique<spdlog::pattern_formatter>(PLAIN_LOG_PATTERN, spdlog::pattern_time_type::local));
        }
    ...
    void monitor_config_thread_func(std::filesystem::path config_path)
    {
        ...
        {
            spdlog::error("Failed to get handle to config directory for monitoring. Dynamic reload disabled. Error: {}",
                          GetLastError());
            return;
        }
        ...
        while (!g_shutdown_flag)
        {
                                      ...
                                      NULL, NULL))
            {
                if (g_shutdown_flag)
                    break;
                Sleep(200);
                spdlog::info("Change detected in config directory, reloading log settings...");
                apply_log_settings(config_path);
            }
            else
            {
                // 当 CancelIoEx 被调用时，ReadDirectoryChangesW 会失败，GetLastError() 返回 ERROR_OPERATION_ABORTED
                // 这正是我们期望的退出方式
                if (g_shutdown_flag || GetLastError() == ERROR_OPERATION_ABORTED)
                {
                    break;
                }
                spdlog::warn("ReadDirectoryChangesW failed. Dynamic reload may be interrupted. Error: {}",
                             GetLastError());
                Sleep(5000);
            }
        }
    ...
    try
    {
        ...
        if (!std::filesystem::exists(config_path))
        {
            ...
            if (default_config.is_open())
            {
                ...
                default_config << "; Log level: trace, debug, info, warn, error, critical, off\n";
                default_config << "level = info\n\n";
                default_config << "; Log format: plain, json\n";
                default_config << "format = plain\n";
            }
        }
        ...
        auto rotating_sink =
            std::make_shared<spdlog::sinks::rotating_file_sink_mt>(file_path.string(), 5 * 1024 * 1024, 10);
        sinks.push_back(rotating_sink);

        auto logger = std::make_shared<spdlog::async_logger>("peshell", begin(sinks), end(sinks), spdlog::thread_pool(),
...
void ShutdownLogger()
{
    ...
    // ==================== [核心修正 3/3] 使用 CancelIoEx 可靠地唤醒线程 ====================
    if (g_hConfigDirHandle != INVALID_HANDLE_VALUE)
    {
        // 这是中断 ReadDirectoryChangesW 阻塞的标准方法
        CancelIoEx(g_hConfigDirHandle, NULL);
    }

    if (g_config_monitor_thread.joinable())
    {
        g_config_monitor_thread.join(); // 现在这里不会再死锁了
    }
    spdlog::shutdown();
}